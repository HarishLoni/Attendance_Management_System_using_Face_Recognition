    ...
    def fill_attendance():
        ...
        try:
            ...
            connection = pymysql.connect(
                host='localhost', user='root', password='Harish@123', db='manually_fill_attendance')
        ...
        if subb == '':
            ...
            def enter_data_DB():
                ...
                if ENROLLMENT == '':
                    ...
                    VALUES = (str(ENROLLMENT), str(STUDENT), str(Date), str(time_now))
                    try:
            ...
            def create_csv():
                ...
                csv_dir = 'StudentDetails'
                # Check if directory exists, if not create it
                if not os.path.exists(csv_dir):
                    os.makedirs(csv_dir)
                
                csv_name = os.path.join(csv_dir, DB_table_name + '.csv')
            ...
            def attf():
                attendance_dir = os.path.join(os.getcwd(), 'Attendance/Manually Attendance/-------Check atttendance-------')
        ...
        try:
            ...
            while True:
                ...
                for (x, y, w, h) in faces:
                    ...
                    filename = os.path.join("TrainingImage", f"{Name}.{Enrollment}.{sampleNum}.jpg")
            ...
            with open('StudentDetails/StudentDetails.csv', 'a+', newline='') as csvFile:
                ...
                if not file_exists:
                    writer.writerow(['Enrollment', 'Name', 'Date', 'Time'])  # Write header
                writer.writerow(row)
                csvFile.close()
            ...
            Notification.configure(text=f"Error: {str(e)}", bg="Red", width=50)
            Notification.place(x=250, y=400)
...
def subjectchoose():
        ...
        if sub == '':
            ...
            try:
                ...
                try:
                    ...
                    Notifica.configure(
                        text=error_msg, bg="red", fg="black", width=33, font=('times', 15, 'bold'))
                ...
                # Make sure StudentDetails directory and file exist
                if not os.path.exists("StudentDetails"):
                    os.makedirs("StudentDetails")
                ...
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        if conf < 70:
                            ...
                            # Fix: Check if Id exists in the dataframe
                ...
                timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                Hour, Minute, Second = timeStamp.split(":")
                fileName = os.path.join("Attendance", f"{Subject}_{date}_{Hour}-{Minute}-{Second}.csv")
                ...
                # Create table for Attendance
                date_for_DB = datetime.datetime.fromtimestamp(ts).strftime('%Y_%m_%d')
                DB_Table_name = str(f"{Subject}_{date_for_DB}_Time_{Hour}_{Minute}_{Second}")
                
                try:
                    connection = pymysql.connect(
                        host='localhost', user='root', password='Harish@123', db='Face_reco_fill')
                    cursor = connection.cursor()
                    
                    sql = f"CREATE TABLE {DB_Table_name} (ID INT NOT NULL AUTO_INCREMENT, ENROLLMENT varchar(100) NOT NULL, NAME VARCHAR(50) NOT NULL, DATE VARCHAR(20) NOT NULL, TIME VARCHAR(20) NOT NULL, PRIMARY KEY (ID));"
                ...
                Notifica.configure(text=error_msg, bg="red", fg="white", width=33, font=('times', 15, 'bold'))
                Notifica.place(x=20, y=250)
    ...
    def Attf():
        attendance_dir = os.path.join(os.getcwd(), 'StudentDetails')
        # Create directory if it doesn't exist
        os.makedirs(attendance_dir, exist_ok=True)
        os.startfile(attendance_dir)
...
def trainimg():
    ...
    try:
        ...
        return  # Added return to prevent further execution if error occurs
    ...
    try:
        ...
        if not os.path.exists("TrainingImageLabel"):
            os.makedirs("TrainingImageLabel")
        recognizer.save("TrainingImageLabel/Trainner.yml")  # Fixed path separator
    except Exception as e:
...
def getImagesAndLabels(path):
    ...
    for imagePath in imagePaths:
        try:
            ...
            # Split the filename properly to get ID
